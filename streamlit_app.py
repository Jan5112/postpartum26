...
if all_df is not None:
    ...
    if st.session_state.view == 'details':
        ...
        st.markdown(f'<div class="recipe-card"><p><span class="meal-label">📅 第 {int(recipe["Day_Int"])} 天 · {recipe["Meal"]}</span></p><hr style="border:1px solid #FFE4E1;"><p><span class="meal-label">🛒 準備食材：</span><br>{recipe["Ingredients"]}</p><p><span class="meal-label">👩‍🍳 烹飪步驟：</span><br>{recipe["Method"]}</p></div>', unsafe_allow_html=True)
    ...
    # C. 🗓️ 每週總覽 (恢復粉紅標籤)
        ...
        for d in range(start, end + 1):
            ...
            if not day_data.empty:
                ...
                st.markdown(f'<div class="week-card"><b style="color:#D87093; font-size:1.1rem;">📅 第 {d} 天</b><br>{items_html}</div>', unsafe_allow_html=True)

    # D. 🛒 採購 Check-list (智能鎖死日期版)
    elif mode == "🛒 採購 Check-list":
        st.markdown("<h1>🛒 採購 Check-list</h1>", unsafe_allow_html=True)
        t1, t2 = st.tabs(["📍 睇今日買乜", "📅 一次過買幾日"])
        with t1:
            s_day = st.number_input("選擇第幾天", 1, 30, 1, key="shop_s")
            # 關鍵修正：嚴格過濾並複製，唔俾佢睇到其他日子
