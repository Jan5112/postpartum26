...
st.markdown("""
    ...
    .usage-text { color: #888888 !important; font-size: 0.9rem; padding: 10px; background: #FFF9FA; border-radius: 10px; margin-bottom: 10px; display: block; }
...
if all_df is not None:
    ...
    if st.session_state.view == 'details':
        ...
        st.markdown('<hr style="border:1px solid #FFE4E1;">', unsafe_allow_html=True)
        ...
        for cat in ["食材", "調味"]:
            if data_sum[cat]:
                ...
                for name, info in data_sum[cat].items():
                    ...
                    with col2:
                        # 使用 Expander 達成：禁個食材名先會出現點解要買
                        label = f"{name} (共 {info['count']} 份)" if info['count'] > 1 else name
    ...
    st.error("載入失敗，請檢查 Google Sheet 連結。")
