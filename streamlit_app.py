...
if all_df is not None:
    ...
    if st.session_state.view == 'details':
        ...
        # 使用 st.write 以保留 Google Sheet 的 \n 換行格式
    ...
    # D. 🛒 採購 Check-list (修正嚴格隔離)
    elif mode == "🛒 採購 Check-list":
        st.markdown("<h1>🛒 採購 Check-list</h1>", unsafe_allow_html=True)
        t1, t2 = st.tabs(["📍 睇今日買乜", "📅 一次過買幾日"])
        with t1:
            s_d = st.number_input("選擇天數", 1, 30, 1, key="s_in_check"); target = [s_d]; u_key = f"s_{s_d}"
        with t2:
            c1, c2 = st.columns(2); r1 = c1.number_input("由 Day", 1, 30, 1); r2 = c2.number_input("至 Day", 1, 30, r1+1)
    ...
    # E. 🔍 食譜大百科 (分類回歸 + 修正重複 Key)
        ...
        if q:
            ...
            for _, row in s_df.iterrows():
                if st.button(f"✨ Day {int(row['Day_Int'])} | {row['Dish_Name']}", key=f"search_btn_{row.name}"):
                    st.session_state.selected_row = row; st.session_state.view = 'details'; st.rerun()
