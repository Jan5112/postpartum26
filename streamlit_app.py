...
st.markdown("""
    ...
    .recipe-card, .check-card, .week-card {
        background-color: white !important; padding: 20px; border-radius: 25px;
...
if all_df is not None:
    ...
    if st.session_state.view == 'details':
        ...
        <div class="recipe-card">
            <p><span class="meal-label">📅 第 {int(r['Day_Int'])} 天 · {r['Meal']}</span></p>
            <hr style="border:1px solid #FFE4E1; margin: 15px 0;">
            <p style="color:#555555;"><b>🛒 準備食材：</b><br>{r['Ingredients']}</p>
            <p style="color:#555555;"><b>👩‍🍳 烹飪步驟：</b><br>{r['Method']}</p>
        </div>
        """, unsafe_allow_html=True)
    ...
    # D. 🛒 採購 Check-list
        ...
        with t1:
            s_d = st.number_input("選擇天數", 1, 30, 1, key="s_in"); target = [s_d]; u_key = f"s_{s_d}"
        with t2:
