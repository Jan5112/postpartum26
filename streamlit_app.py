...
def get_shopping_summary(df_to_process, target_days):
    ...
    for _, row in clean_df.iterrows():
        ...
        for raw in items:
            ...
            # 【核心修正】只將屬於 target_days 內的詳細資訊加入
...
if all_df is not None:
    ...
    if st.session_state.view == 'details':
        ...
        # 全部包進 recipe-card div 內
        content = f"""
        <div class="recipe-card">
            <span class="meal-label">📅 第 {int(r['Day_Int'])} 天 · {r['Meal']}</span>
            <hr style="border:1px solid #FFE4E1; margin: 15px 0;">
            <p><span class="meal-label">🛒 準備食材：</span><br>{str(r['Ingredients']).replace('\\n', '<br>')}</p>
            <p><span class="meal-label">👩‍🍳 烹飪步驟：</span><br>{str(r['Method']).replace('\\n', '<br>')}</p>
        </div>
        """
    ...
    # D. 🛒 採購 Check-list (互動式 + 絕對日期過濾)
    elif mode == "🛒 採購 Check-list":
        st.markdown("<h1>🛒 採購 Check-list</h1>", unsafe_allow_html=True)
        t1, t2 = st.tabs(["📍 睇今日買乜", "📅 一次過買幾日"])
        with t1:
            s_d = st.number_input("天數", 1, 30, 1, key="s_day"); target = [s_d]
        with t2:
            c1, c2 = st.columns(2); r1 = c1.number_input("由", 1, 30, 1); r2 = c2.number_input("至", 1, 30, r1+1)
        ...
        for cat in ["食材", "調味"]:
            if data_sum[cat]:
                ...
                for name, info in data_sum[cat].items():
                    # 統計出現在幾道菜 (count)
                    final_count = len(info["details"])
                    label = f"{name} (共 {final_count} 份)" if final_count > 1 else name
